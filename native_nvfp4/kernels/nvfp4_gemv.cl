...
// Kernel-lab entry points. These intentionally leave the production decode
// kernel unchanged while allowing the host to sweep activation-tile width and
// the number of rows sharing each tile on the actual device. The two entry
// points differ only in the inner 16-value NVFP4 block implementation.
static inline float nvfp4_lab_block_scalar(
    global const uchar * q,
    local const float * xv,
    float d
) {
    float sum = 0.0f;
    #pragma unroll
    for (int j = 0; j < QK_NVFP4_SUB/2; ++j) {
        const uchar qv = q[j];
        sum += d*xv[2*j]*kvalues_nvfp4_f[qv & 0x0F];
        sum += d*xv[2*j + 1]*kvalues_nvfp4_f[qv >> 4];
    }
    return sum;
}

static inline float nvfp4_lab_block_vector(
    global const uchar * q,
    local const float * xv,
    float d
) {
...
static inline void nvfp4_native_gemv_rows_lab_impl(
    global const uchar * packed,
    global const uchar * scales,
    global const float * x,
    global float * dst,
    ...
    const int subgroup = get_sub_group_id();
    const int row_tile = get_num_sub_groups();
    const int lane = get_sub_group_local_id();
    const int row = get_group_id(0)*row_tile + subgroup;
    const int local_thread = get_local_id(0);
    const int packed_stride = cols/2;
    const int scale_stride = cols/QK_NVFP4_SUB;
    const int safe_row = row < rows ? row : 0;
    ...
    for (int tile_base = 0; tile_base < cols; tile_base += k_tile) {
        ...
        for (int i = local_thread; i < tile_cols;
             i += NVFP4_SUBGROUP_WIDTH*row_tile) {
        ...
        for (int tile_block = lane; tile_block < tile_blocks;
            ...
            const int block = tile_base/QK_NVFP4_SUB + tile_block;
            if (row < rows && block < scale_stride) {
                const float d = e4m3_scale_to_fp32(sr[block]) *
                    inv_weight_global_scale;
                global const uchar * q = wr + block*(QK_NVFP4_SUB/2);
                local const float * xv = x_tile + tile_block*QK_NVFP4_SUB;
                sum += vector_decode
                    ? nvfp4_lab_block_vector(q, xv, d)
                    : nvfp4_lab_block_scalar(q, xv, d);
            }
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
...
__attribute__((qcom_reqd_sub_group_size("half")))
kernel void nvfp4_native_gemv_rows_lab_scalar(
    global const uchar * packed,
    global const uchar * scales,
    global const float * x,
    global float * dst,
    ...
    nvfp4_native_gemv_rows_lab_impl(
        packed, scales, x, dst, cols, rows, inv_weight_global_scale,
        k_tile, x_tile, 0);
}

__attribute__((qcom_reqd_sub_group_size("half")))
kernel void nvfp4_native_gemv_rows_lab_vector(
    global const uchar * packed,
    global const uchar * scales,
    global const float * x,
    global float * dst,
...
static inline float nvfp4_lab_block_vector_global(
    global const uchar * q,
...
static inline void nvfp4_native_gemv_rows_lab_direct_impl(
    global const uchar * packed,
    global const uchar * scales,
    global const float * x,
    global float * dst,
    ...
    for (int block = lane; block < scale_stride;
        ...
        const float d = e4m3_scale_to_fp32(sr[block]) *
            inv_weight_global_scale;
        global const uchar * q = wr + block*(QK_NVFP4_SUB/2);
        global const float * xv = x + block*QK_NVFP4_SUB;
        if (vector_decode) {
            sum += nvfp4_lab_block_vector_global(q, xv, d);
        } else {
            #pragma unroll
            for (int j = 0; j < QK_NVFP4_SUB/2; ++j) {
                const uchar qv = q[j];
...
__attribute__((qcom_reqd_sub_group_size("half")))
kernel void nvfp4_native_gemv_rows_lab_direct_scalar(
    global const uchar * packed,
    global const uchar * scales,
    global const float * x,
    global float * dst,
    ...
    nvfp4_native_gemv_rows_lab_direct_impl(
        packed, scales, x, dst, cols, rows, inv_weight_global_scale, 0);
}

__attribute__((qcom_reqd_sub_group_size("half")))
kernel void nvfp4_native_gemv_rows_lab_direct_vector(
    global const uchar * packed,
    global const uchar * scales,
    global const float * x,
    global float * dst,
    ...
    nvfp4_native_gemv_rows_lab_direct_impl(
        packed, scales, x, dst, cols, rows, inv_weight_global_scale, 1);
}
